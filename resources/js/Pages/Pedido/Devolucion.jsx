...
export default function Devolucion() {
  ...
  const manejarEnvio = (e) => {
    ...
    if (!datos.pedido.trim())
      nuevosErrores.pedido = "Debes seleccionar un pedido.";
    ...
    if (Object.keys(nuevosErrores).length === 0) {
      // Lo convertimos en array para poder comparar
      router.post(route("devoluciones.guardar"), datos, {
        onSuccess: () => {
          setDatos({
            nombre: user.name,
            pedido: "",
      ...
      <main>
        ...
        <section className="bg-[#040A2A] text-white py-10 px-6 pt-20">
          <div className="max-w-5xl mx-auto">
            <div className="mb-10">
              <h1 className="text-3xl font-bold relative w-fit z-10">
            ...
            <div className="flex flex-wrap gap-10">
              ...
              >
                  ...
                  >
                    ...
                    {pedidos.map((p) => (
                      <option key={p.id} value={p.numero_factura}>
                        Pedido #{p.numero_factura} - {p.created_at.slice(0, 10)}{" "}
                        {/* -> mostrar año mes y dia */}
                      </option>
                    ))}
                  </select>
                  {errores.pedido && (
                    <p className="text-red-400 text-sm">{errores.pedido}</p>
                  )}
                </label>
                ...
                <label className="block">
                  ...
                  />
                  {errores.telefono && (
                    <p className="text-red-400 text-sm">{errores.telefono}</p>
                  )}
                </label>
                ...
                <label className="block">
                  ...
                  />
                  {errores.mensaje && (
                    <p className="text-red-400 text-sm">{errores.mensaje}</p>
                  )}
                </label>
                ...
                <div className="flex flex-col gap-2">
                  <label className="flex gap-2 items-start">
                    ...
                    <span className="text-sm">
                      Acepto recibir comunicaciones vía email y teléfono para
                      atender a mi solicitud
                    </span>
                  </label>
                  {errores.acepta && (
                    <p className="text-red-400 text-sm">{errores.acepta}</p>
                  )}
                </div>

                <Boton tipo="submit" texto="ENVIAR" color="red" tamaño="md" />
              </form>
              ...
              <div className="flex-1 min-w-[260px] relative space-y-3">
                <div
                  ...
                  style={{
                    backgroundImage: "url(/images/perez-motor/LOGO.png)",
                  }}
                ></div>
