...
export default function Edit({ mantenimiento, tiposMantenimiento }) {
  ...
  const detallesIniciales = mantenimiento.mantenimiento_detalles.map(d => ({
    tipo_mantenimiento_id: d.tipo_mantenimiento_id,
    limpiar: d.limpiar,
    revisar: d.revisar,
    sustituir: d.sustituir,
    precio: d.precio,
  }));

  const { data, setData, put, processing, errors, reset } = useForm({
    vehiculo_id: mantenimiento.vehiculo.id,
    cita_taller_id: mantenimiento.cita_taller_id,
    kilometros: mantenimiento.kilometros,
    observaciones: mantenimiento.observaciones ?? "",
    prox_revision: mantenimiento.prox_revision ?? "",
    mano_obra: mantenimiento.mano_obra,
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    put(route("admin.mantenimientos.update", mantenimiento.id), {
      onSuccess: () => reset(),
    });
  };
      ...
      <section className="min-h-screen bg-gradient-to-br from-[#040A2A] to-[#232b4b] text-white py-12 px-2 flex flex-col items-center">
        <div className="max-w-3xl w-full mx-auto bg-white/95 rounded-xl shadow-lg p-10 border border-gray-100 relative">
          <h1 className="text-3xl font-black text-[#141b3a] mb-8 flex items-center gap-4">
            <Plus className="w-7 h-7 text-[#C42424]" /> Editar mantenimiento
          </h1>
          ...
          <div className="mb-6 flex flex-wrap gap-6 justify-between bg-gradient-to-l from-pink-100 via-white to-blue-100 border-l-4 border-[#C42424] p-5 rounded-2xl shadow">
            <div>
              ...
              <div className="text-gray-500 font-mono mb-1">{mantenimiento.vehiculo.matricula}</div>
              {mantenimiento.vehiculo.vin && (
                <div className="text-gray-700 text-xs font-semibold">
                  <span className="uppercase text-[#C42424]">Bastidor:</span> {mantenimiento.vehiculo.vin}
                </div>
              )}
            </div>
            <div className="text-[#C42424] font-semibold text-lg">
              {mantenimiento.cita
                ? (
                    <>
                      Cita:{" "}
                      {mantenimiento.cita?.fecha
                        ? new Date(mantenimiento.cita?.fecha).toLocaleDateString("es-ES")
                        : ""}
                      {mantenimiento.cita.hora &&
                        ` - ${mantenimiento.cita?.hora.slice(0, 5)}`}
                    </>
                  )
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="mt-8 flex justify-end">
              <Boton
                texto={processing ? "Guardando..." : "Guardar cambios"}
                color="green"
                tamaño="lg"
                tipo="submit"
                className="px-8 py-3"
